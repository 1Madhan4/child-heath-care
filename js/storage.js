...
const Storage = {
    // Initialize or get DB from localStorage
    getDB() {
        try {
    ...
    // Helper to sanitize emails for keys
    sanitizeEmail(email) {
    ...
    getSession() {
        try {
            ...
            if (!data) return null;
            // ── Security: Auto-expire sessions after 24 hours ──
    ...
    async saveUserProfile(userConfig) {
        const db = this.getDB();
        const key = this.sanitizeEmail(userConfig.email);
